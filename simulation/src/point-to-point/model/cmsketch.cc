...
namespace ns3
{
    ...
    int CountMinSketch::cms_init_optimal_alt(CountMinSketch* cms, double error_rate, double confidence, cms_hash_function hash_function) {
        ...
        int32_t width = ceil(2 / error_rate);
        int32_t depth = ceil((-1 * log(1 - confidence)) / LOG_TWO);
        return __setup_cms(cms, width, depth, error_rate, confidence, hash_function);
    }

    int CountMinSketch::cms_init_alt(CountMinSketch* cms, int32_t width, int32_t depth, cms_hash_function hash_function) {
        if (depth < 1 || width < 1) {
    ...
    int CountMinSketch::cms_clear(CountMinSketch* cms) {
        uint64_t i, j = cms->width * cms->depth;
        for (i = 0; i < j; ++i) {
            cms->bins[i] = 0;
        }
        cms->elements_added = 0;
    ...
    uint64_t CountMinSketch::cms_add_inc_alt(CountMinSketch* cms, uint64_t* hashes, unsigned int num_hashes, uint64_t x) {
    ...
    uint64_t CountMinSketch::cms_add_inc(CountMinSketch* cms, const char* key, uint64_t x) {
        uint64_t* hashes = cms_get_hashes(cms, key);
        uint64_t num_add = cms_add_inc_alt(cms, hashes, cms->depth, x);
        free(hashes);
        return num_add;
    }

    uint64_t CountMinSketch::cms_remove_inc_alt(CountMinSketch* cms, uint64_t* hashes, unsigned int num_hashes, uint64_t x) {
        if (num_hashes < cms->depth) {
            fprintf(stderr, "Insufficient hashes to complete the removal of the element to the count-min sketch!");
            return CMS_ERROR;
        }
        uint64_t num_add = INT32_MAX;
        for (unsigned int i = 0; i < cms->depth; ++i) {
            uint64_t bin = (hashes[i] % cms->width) + (i * cms->width);
    ...
    uint64_t CountMinSketch::cms_remove_inc(CountMinSketch* cms, const char* key, uint64_t x) {
        uint64_t* hashes = cms_get_hashes(cms, key);
        uint64_t num_add = cms_remove_inc_alt(cms, hashes, cms->depth, x);
        free(hashes);
        return num_add;
    }

    uint64_t CountMinSketch::cms_check_alt(CountMinSketch* cms, uint64_t* hashes, unsigned int num_hashes) {
        if (num_hashes < cms->depth) {
    ...
    uint64_t CountMinSketch::cms_check(CountMinSketch* cms, const char* key) {
        uint64_t* hashes = cms_get_hashes(cms, key);
        uint64_t num_add = cms_check_alt(cms, hashes, cms->depth);
        free(hashes);
        return num_add;
    }

    uint64_t CountMinSketch::cms_check_mean_alt(CountMinSketch* cms, uint64_t* hashes, unsigned int num_hashes) {
        if (num_hashes < cms->depth) {
    ...
    uint64_t CountMinSketch::cms_check_mean(CountMinSketch* cms, const char* key) {
        uint64_t* hashes = cms_get_hashes(cms, key);
        uint64_t num_add = cms_check_mean_alt(cms, hashes, cms->depth);
        free(hashes);
        return num_add;
    }

    uint64_t CountMinSketch::cms_check_mean_min_alt(CountMinSketch* cms, uint64_t* hashes, unsigned int num_hashes) {
        if (num_hashes < cms->depth) {
    ...
    uint64_t CountMinSketch::cms_check_mean_min(CountMinSketch* cms, const char* key) {
        uint64_t* hashes = cms_get_hashes(cms, key);
        uint64_t num_add = cms_check_mean_min_alt(cms, hashes, cms->depth);
        free(hashes);
        return num_add;
    }
